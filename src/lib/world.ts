...
const definitions: Omit<WorldRegion, "checkpoints">[] = [
  ...
  { id: "realm-of-ascension", name: "Realm of Ascension", levelStart: 11, levelEnd: 15, theme: "Transcendence. Mastery. Elevation.", landmark: "Ascension Throne", characterStage: "Ascendant", status: "available", terrainLayers: ["paving", "rock", "moss"], atmosphere: { fog: "#A8C0D2", density: .0045, sunIntensity: 2.8 }, requiredAssets: [] },
...
export const WORLD_REGIONS: WorldRegion[] = definitions.map((region, index) => ({
  ...
  checkpoints: names[index].map((name, local) => {
    ...
    return {
      ...
      worldPosition: index === 0 ? shorePositions[local] : index === 1 ? forestPositions[local] : index === 2 ? realmPositions[local] : [0, 0, -local * 5],
      heroRotation: Math.PI,
      cameraOffset: index === 2 ? realmCameraOffsets[local] : index === 1 ? [0, 3.3, 9.8] : [0, 2.8, 8.2],
      mapPosition: [mapCenters[index] + Math.sin(local * 1.3 + index) * 65, 1400 - index * 205 - local * 33],
      nextCheckpoint: level < 26 ? level + 1 : null,
    };
  }),
}));
