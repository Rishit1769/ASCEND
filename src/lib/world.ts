...
const definitions: Omit<WorldRegion, "checkpoints">[] = [
  ...
  { id: "forest-of-resolve", name: "Forest of Resolve", levelStart: 6, levelEnd: 10, theme: "Discipline. Consistency. Persistence.", landmark: "Guardian Tree and Gate", characterStage: "Warrior", status: "assets-required", terrainLayers: ["forest-floor", "mud", "moss", "rock"], atmosphere: { fog: "#708477", density: .032, sunIntensity: 2.2 }, requiredAssets: ["Monumental ancient tree with roots and canopy LODs", "Forest terrain and stream banks", "Guardian gate, shrine and old bridge", "Leaf litter PBR set"] },
  { id: "mountains-of-trial", name: "Mountains of Trial", levelStart: 11, levelEnd: 15, theme: "Difficulty. Pressure. Endurance.", landmark: "Mountain Temple", characterStage: "Champion", status: "assets-required", terrainLayers: ["rock", "gravel", "moss", "snow"], atmosphere: { fog: "#8b9eac", density: .016, sunIntensity: 3.2 }, requiredAssets: ["Traversable mountain trail terrain with collision", "Broken bridge and mountain temple", "Waterfall and scree PBR set"] },
...
export const WORLD_REGIONS: WorldRegion[] = definitions.map((region, index) => ({
  ...
  checkpoints: names[index].map((name, local) => {
    ...
    return {
      ...
      worldPosition: index === 0 ? shorePositions[local] : [0, 0, -local * 5],
