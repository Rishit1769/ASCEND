...
export default function Ruins() {
  ...
  const plants = useMemo(() => CLUSTERS.flatMap(([x, z], cluster) =>
    Array.from({ length: preset === "ultra" || preset === "high" ? 15 : preset === "medium" ? 9 : 4 }, (_, i) => {
  ...
  const stones = useMemo(() => Array.from({ length: preset === "low" || preset === "potato" ? 16 : 40 }, (_, i) => {
    const z = 2 - i * .48;
  ...
  return (
    <group name="shore-architecture-and-vegetation">
      ...
      <GroundedAsset id="tree_small_02" height={9.5} position={[-8, 0, -13]} rotation={1.6} burial={.012} castShadow={preset !== "low" && preset !== "potato"} />
      ...
      {preset !== "low" && preset !== "potato" && <GroundedAsset id="tree_small_02" low height={5.6} position={[8, 0, -25]} rotation={.8} burial={.012} />}
    </group>
  );
}
